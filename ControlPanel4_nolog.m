...
function [time, idiff, tripped, tripTime, conv87T, hybrid] = generateRealisticData(scenario)
    ...
    %--- Realistic multi-layer noise model (field-calibrated) ----------------
    %  Sources modelled (per substation EMC environment):
    %   1. Thermal/Johnson broadband white noise
    %   2. CT ratio-error noise  (IEC 61869-2 Class 0.5, ≤0.5 % ratio error)
    %   3. 1/f pink noise from relay electronics (FFT spectral shaping)
    %   4. Conducted EMI: odd harmonics 3rd–13th + SMPS inter-harmonics
    %      (IEC 61000-4-7 Class II substation environment)
    %   5. 60 Hz cross-circuit induction from adjacent feeders
    %   6. ADC quantisation noise (16-bit, 10 pu full-scale)
    %  Arc-ionisation burst noise is added per-scenario after signal assembly.
    %------------------------------------------------------------------------
    nPts = length(t);

    % 1. Thermal (Johnson) broadband white noise  [≈0.8 % rated]
    thermal   = 0.008 * randn(nPts, 3);

    % 2. CT ratio-error noise — random per-phase gain mismatch
    ct_err    = 0.003 + 0.002 * rand(1, 3);        % 0.3–0.5 % per phase
    ct_noise  = ct_err .* randn(nPts, 3);

    % 3. 1/f (pink) noise — relay electronics low-frequency drift
    fvec      = max((0:nPts-1)' * (fs/nPts), 1);   % avoid DC singularity
    pink_shp  = 1 ./ sqrt(fvec); pink_shp(1) = 0;  % suppress DC
    pink_raw  = real(ifft(fft(randn(nPts, 3)) .* pink_shp));
    pink_nois = 0.003 * pink_raw ./ (std(pink_raw) + 1e-12);

    % 4. Conducted EMI harmonics + SMPS inter-harmonics
    phi_h = rand(9, 3) * 2*pi;
    emi   = 0.006*sin( 3*w*t + phi_h(1,:)) + 0.004*sin( 5*w*t + phi_h(2,:)) ...
          + 0.003*sin( 7*w*t + phi_h(3,:)) + 0.002*sin( 9*w*t + phi_h(4,:)) ...
          + 0.0015*sin(11*w*t + phi_h(5,:))+ 0.001*sin(13*w*t + phi_h(6,:)) ...
          + 0.002*sin(2*pi* 75*t + phi_h(7,:))  ...  % SMPS inter-harmonics
          + 0.0015*sin(2*pi*125*t + phi_h(8,:)) ...
          + 0.001*sin(2*pi*175*t + phi_h(9,:));

    % 5. 60 Hz cross-circuit induction (adjacent feeder coupling)
    phi60   = rand(1, 3) * 2*pi;
    cross60 = 0.003 * sin(2*pi*60*t + phi60);

    % 6. ADC quantisation  (16-bit, 10 pu full-scale  =>  LSB ≈ 1.5e-4 pu)
    quant   = (10/2^16) * (rand(nPts, 3) - 0.5);

    noise = thermal + ct_noise + pink_nois + emi + cross60 + quant;
    ...
    % -------- Arc-ionisation burst noise (fault inception only) -------------
    %  Models HF arc re-ignition noise in the first ~9 ms after fault onset.
    %  Exponentially-decaying Gaussian burst; absent for Normal & Inrush.
    isFaultScenario = ismember(scenario, {'InternalAG','InternalBG','InternalAB', ...
                                           'InternalABC','External','ExternalAB'});
    if isFaultScenario
        arc_tau = 0.0015;                                % 1.5 ms decay constant
        arc_win = (t >= tf) & (t < tf + 6*arc_tau);
        arc_env = zeros(nPts, 1);
        arc_env(arc_win) = exp(-(t(arc_win) - tf) / arc_tau);
        idiff = idiff + 0.018 * randn(nPts, 3) .* arc_env;
    end
