...
% ----------------------------------------------------------------------------
%  Breaker / flux configuration helpers (added for realistic inrush handling)
% ----------------------------------------------------------------------------
function setBreakersClosed(model)
    % Default state for NON-inrush scenarios:
    %   - Both breakers controlled by the Hybrid 87T Relay (External: on)
    %   - Both start CLOSED, no internal time toggles
    %   - Transformer residual flux disabled
...
function setInrushEnergization(model, residualFlux)
    % Configure HV breaker to start OPEN and close at t = 50 ms (50 Hz / 1 cycle settle).
    % Apply randomized residual flux on the transformer core so the closing instant
    % sees a non-zero core flux  =>  authentic magnetising inrush.
    % LV breaker stays closed  =>  load remains connected (field-realistic).
    if nargin < 2 || isempty(residualFlux)
        residualFlux = (rand(1,3)*2 - 1) * 0.80;   % U[-0.8, +0.8] pu per phase
    end
    hv = [model '/Three-Phase Breaker '];
    lv = [model '/Three-Phase Breaker1 '];
    try
        set_param(hv, 'External', 'off');          % use internal SwitchTimes
        set_param(hv, 'InitialState', 'open');
        set_param(hv, 'SwitchTimes', '[0.050]');   % single close at 50 ms
    catch ME, warning('ControlPanel:HVBreaker', '%s', ME.message); end
    try
        set_param(lv, 'External', 'on');           % keep relay-controlled
        set_param(lv, 'InitialState', 'closed');
        set_param(lv, 'SwitchTimes', '[]');
    catch ME, warning('ControlPanel:LVBreaker', '%s', ME.message); end
    try
...
function configureInrushScenario(model)
    resetFaultBlocks(model);
    setInrushEnergization(model);   % HV breaker opens at t=0, closes at t=50ms; randomised residual flux applied
end
