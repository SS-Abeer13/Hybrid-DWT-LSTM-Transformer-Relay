function ControlPanel_StressTest_Topup
% =========================================================================
% TRANSFORMER PROTECTION — TARGETED TOP-UP DATASET GENERATOR
% =========================================================================
%
% PURPOSE
%   Generates ONLY the scenario types that failed in the first batch run
%   (38.7% failure rate from the original ControlPanel_StressTest run).
%   Use this to fill coverage gaps without re-running the full distribution.
%
%   After running, combine both .mat files with CombineAndExtractFeatures.m.
%
% TARGETED SCENARIOS (previously 70–100% failure rate)
%   Internal  ABG, ACG, BCG, CG               — multi-phase ground faults
%   Internal  HiZ BG, CG, BCG, ABG, ABCG      — HiZ multi-phase
%   Internal  HiZ AG (partial), HiZ ABC        — HiZ high-fail
%   Internal  HiZ ACG (partial)                — HiZ partial-fail
%   Internal  Evolving                         — evolving fault sequence
%   External  AB, ABC                          — ungrounded multi-phase
%   External  AG, ACG (partial), ABCG (partial)— phase-A ground externals
%
% EQUAL WEIGHTING: all 17 targeted scenario types receive equal sample counts
% for maximum gap-filling efficiency per simulation minute.
%
% USAGE
%   1. Open your Simulink model.
%   2. Run:  ControlPanel_StressTest_Topup
%   3. Set sample count (recommended: 3000–5000) and click GENERATE.
%   4. Combine with existing StressTestDataset_*.mat via CombineAndExtractFeatures.m
%
...
% =========================================================================
    ...
    % ── Figure ────────────────────────────────────────────────────────────
    f = figure('Name','Stress-Test TOPUP Generator (Gap-Filling)','NumberTitle','off', ...
    ...
    uicontrol('Parent',f,'Style','text', ...
              ...
              'String',['Model: ' modelName '   |   GAP-FILL: 17 previously-failed scenarios, equal weighting'], ...
...
function generateStressBatch(model, hN, hT, hSaveViz)
    ...
    try
        ...
        filename  = sprintf('StressTestDataset_TOPUP_%s.mat', timestamp);
...
% =========================================================================
% SCENARIO TABLE  — ONLY the 17 scenarios that failed in the first batch
% =========================================================================
function ST = buildScenarioTable()
    % Equal weight (5) on every scenario so each gets the same sample count.
    % Scenarios are the ones with ≥44% failure rate in the first batch run.
    %
    % Entry:  name | zone | block | fA fB fC fG | Rf_lo Rf_hi | shouldTrip | weight
    rows = {
    %  name                      zone       block               fA fB fC fG  Rlo   Rhi   trip   wt
    %  ── Internal multi-phase ground faults (100% failed) ────────────────────────────
      'Internal_ABG'          'Internal' 'Internal_Fault1'    1  1  0  1  0.01  2.0   true    5
      'Internal_ACG'          'Internal' 'Internal_Fault1'    1  0  1  1  0.01  2.0   true    5
      'Internal_BCG'          'Internal' 'Internal_Fault1'    0  1  1  1  0.01  2.0   true    5
      'Internal_CG'           'Internal' 'Internal_Fault1'    0  0  1  1  0.01  2.0   true    5
    %  ── Internal HiZ (100% failed) ───────────────────────────────────────────────
      'Internal_HiZ_BG'       'Internal' 'Internal_Fault1'    0  1  0  1  50.0  300   true    5
      'Internal_HiZ_CG'       'Internal' 'Internal_Fault1'    0  0  1  1  50.0  300   true    5
      'Internal_HiZ_ABG'      'Internal' 'Internal_Fault1'    1  1  0  1  50.0  300   true    5
      'Internal_HiZ_BCG'      'Internal' 'Internal_Fault1'    0  1  1  1  50.0  300   true    5
      'Internal_HiZ_ABCG'     'Internal' 'Internal_Fault1'    1  1  1  1  50.0  300   true    5
    %  ── Internal HiZ (71–74% failed) ────────────────────────────────────────────
      'Internal_HiZ_AG'       'Internal' 'Internal_Fault1'    1  0  0  1  50.0  300   true    5
      'Internal_HiZ_ABC'      'Internal' 'Internal_Fault1'    1  1  1  0  50.0  300   true    5
      'Internal_HiZ_ACG'      'Internal' 'Internal_Fault1'    1  0  1  1  50.0  300   true    5
    %  ── Internal evolving fault (100% failed) ────────────────────────────────────
      'Internal_Evolving'     'Internal' 'evolving'           1  0  0  1  0.01  0.5   true    5
    %  ── External ungrounded multi-phase (100% failed) ────────────────────────────
      'External_AB'           'External' 'External_Fault'     1  1  0  0  0.01  5.0   false   5
      'External_ABC'          'External' 'External_Fault'     1  1  1  0  0.01  5.0   false   5
    %  ── External phase-A ground (87% failed) ─────────────────────────────────────
      'External_AG'           'External' 'External_Fault'     1  0  0  1  0.01  5.0   false   5
      'External_ACG'          'External' 'External_Fault'     1  0  1  1  0.01  5.0   false   5
    };
...
function plotDiagnostics(dataset, N, ST, counts, saveViz, timestamp)
    ...
    fig1 = figure('Name','Stress-Test: Dataset Composition', ...
                  'Position',[50 100 1280 780],'Color',[0.97 0.97 0.97]);
    ...
    fig2 = figure('Name','Stress-Test: Parameter Space Coverage', ...
                  'Position',[80 60 1280 820],'Color',[0.97 0.97 0.97]);
    ...
    fig3 = figure('Name','Stress-Test: Statistical Verification', ...
                  'Position',[110 40 1280 820],'Color',[0.97 0.97 0.97]);
